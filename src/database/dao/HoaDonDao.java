...
//    }
	...
	public KhachHang getKhachHangTheoHD(String ma){
		...
		try {
			Connection connection = ConnectDB.getConnection();
			// Sửa lỗi: Thêm khoảng trắng giữa các từ trong chuỗi SQL
			...
			st.setString(1, ma);
			ResultSet rs = st.executeQuery();
			if (rs.next()){  // Thay while bằng if vì chỉ có 1 khách hàng
				String maKH = rs.getString("maKH");
				String tenKH = rs.getString("tenKH");
				LocalDate ngaySinh = rs.getDate("ngaySinh").toLocalDate();
				Boolean gioiTinh = rs.getBoolean("gioiTinh");
...
//        String sql="select top 1 hd.maHD,hd.ngayLap,hd.phuongThucTT,hd.trangThaiHD,hd.tienNhan,"
//                + "kh.maKH,kh.tenKH,kh.sdt,kh.soCCCD,nv.maNV,nv.tenNV "
...
//                hd=new HoaDon(maHD,ngayLap,pttt,trangThaiHD,kh,dsCTDH,nv,tienNhan);
//                hd.setTongTien(); hd.setTienThue(); hd.setTongTienThanhToan();
//            }
...
//                String maHD, LocalDateTime ngayLap, PhuongThucThanhToan pTTT, TrangThaiHoaDon trangThai,
